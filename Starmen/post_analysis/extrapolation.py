...
from Starmen.lib.dataset import Data_preprocess_starmen, Dataset_starmen
...
def extrapolate(image, missing_num):
    ...
    b = torch.matmul(
        torch.inverse((autoencoder.sigma0_2 + autoencoder.sigma2_2) * yty - 2 * autoencoder.sigma0_2
                      * autoencoder.sigma2_2 * torch.inverse(autoencoder.D)),
...
if __name__ == '__main__':
    ...
    autoencoder = torch.load('../trained_model/pretrained_UOMETM', map_location=device)
    autoencoder.eval()
    autoencoder.Training = False
    autoencoder.device = device

    # load train and test data
    train_data, test_data = data_generator.generate_train_test(fold=0)
    ...
    with torch.no_grad():
        for data in test_loader:
            ...
            extra_loss = extrapolate(image, missing_num=np.floor(input_para.mr * 10))
            extra_losses += float(extra_loss)

    print(extra_losses / batches / 64 / 64)