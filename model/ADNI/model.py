...
class UOMETM(nn.Module):
    def __init__(self, dim_z, input_dim, left_right=0):
    ...
    def _init_mixed_effect_model(self):
        self.beta = torch.rand(size=[self.X.size()[1], self.dim_z], device=self.device)
        self.b = torch.normal(mean=0, std=1, size=[self.Y.size()[1], self.dim_z], device=self.device)
    ...
    def train_(self, train_data_loader, test_data_loader, optimizer, num_epochs):
        ...
        for epoch in range(num_epochs):
            ...
            if epoch > 30:
                ...
                print('Mixed-effects modeling finished...')
    ...
    def mixed_effects_modeling(self, Z, ZU, ZV):
        ...
        for epoch in range(10):
            ...
            self.sigma0_2 = 1 / (N * self.dim_z) * torch.pow(torch.norm(Z - xbeta - yb, p='fro'), 2)
            self.sigma1_2 = 1 / (N * self.dim_z) * torch.pow(torch.norm(ZU - xbeta, p='fro'), 2)
            self.sigma2_2 = 1 / (N * self.dim_z) * torch.pow(torch.norm(ZV - yb, p='fro'), 2)
